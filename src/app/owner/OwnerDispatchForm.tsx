...
import { confirmOwnerDispatch, overrideOwnerDispatch, replaceOwnerDispatch, overrideReplaceOwnerDispatch } from "../actions";
...
export function OwnerDispatchForm({ orderId, orderVersion, requestedAt, durationMinutes = 60, candidates, selectedEmployeeId, onCancelSelection, oldAssignmentId }: { orderId: string; orderVersion: string; requestedAt: string; durationMinutes?: number; candidates: Candidate[]; selectedEmployeeId?: string; onCancelSelection?: () => void; oldAssignmentId?: string }) {
  const initialState = { message: "", ok: false };
  const [confirmState, confirmAction] = useActionState(oldAssignmentId ? replaceOwnerDispatch : confirmOwnerDispatch, initialState);
  const [overrideState, overrideAction] = useActionState(oldAssignmentId ? overrideReplaceOwnerDispatch : overrideOwnerDispatch, initialState);
    ...
    <div className={`mt-3 space-y-3 border-t border-slate-200 pt-3 ${isSelected ? "block" : "hidden"}`}>
      {!isOverride ? (
        <form action={confirmAction} className="flex flex-wrap items-center gap-2">
          <Fields orderId={orderId} orderVersion={orderVersion} requestedAt={requestedAt} durationMinutes={durationMinutes} candidates={candidates} selectedEmployeeId={selectedEmployeeId} oldAssignmentId={oldAssignmentId} />
        ...
        <form action={overrideAction} className="rounded-md border border-amber-200 bg-amber-50 p-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Fields orderId={orderId} orderVersion={orderVersion} requestedAt={requestedAt} durationMinutes={durationMinutes} candidates={candidates} selectedEmployeeId={selectedEmployeeId} reason={reason} onReasonChange={setReason} oldAssignmentId={oldAssignmentId} />
...
function Fields({ orderId, orderVersion, requestedAt, durationMinutes, candidates, selectedEmployeeId, reason, onReasonChange, oldAssignmentId }: { orderId: string; orderVersion: string; requestedAt: string; durationMinutes: number; candidates: Candidate[]; selectedEmployeeId?: string; reason?: string; onReasonChange?: (value: string) => void; oldAssignmentId?: string }) {
  ...
  return <>
    ...
    {oldAssignmentId && <input type="hidden" name="oldAssignmentId" value={oldAssignmentId} />}
