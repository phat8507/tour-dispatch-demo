...
export function OwnerDispatchForm({ orderId, orderVersion, requestedAt, candidates, selectedEmployeeId }: { orderId: string; orderVersion: string; requestedAt: string; candidates: Candidate[]; selectedEmployeeId?: string }) {
  ...
  return <div className="mt-3 grid gap-2"><form action={confirmAction}><Fields key={`confirm-${selectedEmployeeId ?? "none"}`} orderId={orderId} orderVersion={orderVersion} requestedAt={requestedAt} candidates={candidates} selectedEmployeeId={selectedEmployeeId} /><OwnerSubmit label="Xác nhận phân công" /></form><form action={overrideAction}><Fields key={`override-${selectedEmployeeId ?? "none"}`} orderId={orderId} orderVersion={orderVersion} requestedAt={requestedAt} candidates={candidates} selectedEmployeeId={selectedEmployeeId} reason /><OwnerSubmit label="Ghi đè phân công" /></form><OwnerDispatchStatus confirmState={confirmState} overrideState={overrideState} /></div>;
}
