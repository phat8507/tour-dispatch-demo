      ...
      {recommendations.length === 0 ? <p className="mt-2 text-xs text-slate-600">Không có ứng viên hợp lệ từ dữ liệu hiện tại.</p> : (
        <div className="mt-2 grid gap-2">
          {recommendations.map((recommendation) => (
            <div key={recommendation.employeeId} className={`rounded-md border p-2.5 text-left text-xs ${selectedEmployeeId === recommendation.employeeId ? "border-blue-500 bg-blue-50" : "border-slate-200 bg-white"}`}>
              ...
              <button type="button" aria-label={`Chọn nhân viên ${recommendation.employeeName}`} aria-pressed={selectedEmployeeId === recommendation.employeeId} onClick={() => onSelect(recommendation.employeeId)} className="mt-2 min-h-11 rounded-md border border-slate-300 bg-white px-3 font-medium text-slate-800 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">Chọn nhân viên</button>
            </div>
          ))}
        </div>
      )}
